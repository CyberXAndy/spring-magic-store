...
public class BuyNowController {
    ...
    @GetMapping("/buyNow")
    @ResponseBody
    public String buyNow(@RequestParam String productID) {
        // Validate and sanitize the product ID parameter
        if (!SecurityUtils.isValidId(productID)) {
            return generateErrorResponse("Invalid product ID provided.");
        }
        ...
        try {
            ...
            return generateErrorResponse("Invalid product ID format.");
        }
    ...
    private String generateErrorResponse(String errorMessage) {
        return "<!DOCTYPE html>" +
                ...
                ".btn-primary { background-color: #6a5acd; border-color: #6a5acd; }" +
                "</style>" +
                "<title>Owl's Secret Magic Store - Error</title>" +
                "</head>" +
                "<body>" +
                "<div class=\"container\">" +
                "<h1>🦉 Owl's Secret Magic Store 🪄</h1>" +
                "<hr>" +
                "<h2>Error!</h2>" +
                "<p>" + SecurityUtils.sanitizeInput(errorMessage) + "</p>" +
                "<a href=\"/mainscreen\" class=\"btn btn-primary\">Back to Main Screen</a>" +
                "</div>" +
                "</body>" +
                "</html>";
